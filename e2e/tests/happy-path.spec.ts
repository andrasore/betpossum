...
test('alice logs in, places a bet, the event resolves, and the bet settles as won', async ({ browser }) => {
  ...
  const resp = await apiContext.post(`${GATEWAY_URL}/admin/events/${eventId}/result`, {
    headers: {
